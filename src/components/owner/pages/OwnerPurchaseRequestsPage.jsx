...
const statusClass = (status) => `owner-combo-status owner-combo-status--${status || "default"}`;

const comboPlaceholder = (name = "Combo") => (name || "C").trim().slice(0, 1).toUpperCase();
...
export default function OwnerPurchaseRequestsPage() {
  ...
  const [form, setForm] = useState({ comboId: "", gymId: "", note: "", contactName: "", contactPhone: "", contactEmail: "" });
  ...
  const comboTotals = (combo) => {
    ...
    return {
      ...
      itemUnits: Array.isArray(combo?.items) ? combo.items.reduce((sum, item) => sum + Number(item.quantity || 0), 0) : 0,
    };
  };
  ...
  const loadRefs = async () => {
    ...
    setForm((prev) => ({
      ...prev,
      gymId: selectedGymId ? String(selectedGymId) : (prev.gymId || String(gymRows?.[0]?.id || "")),
  ...
  const loadRequests = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await ownerGetPurchaseRequests({ page: 1, limit: 50, gymId: selectedGymId || undefined });
  ...
  useEffect(() => {
    ...
    const exists = requests.some((item) => Number(item.id) === purchaseRequestId);
    if (!exists) return;

    window.requestAnimationFrame(() => {
      const el = document.getElementById(`owner-purchase-request-${purchaseRequestId}`);
  ...
  const payPhase = async (requestId, phase) => {
    ...
    try {
      const res = await ownerCreatePurchaseRequestPayOSLink(requestId, { phase });
  ...
  const renderEquipmentRows = (items = [], dense = false) => (
    <div className={`owner-combo-equipmentList ${dense ? "is-dense" : ""}`}>
      {items.map((item, index) => {
        ...
        return (
          <div key={item.id || `${item.equipmentId}-${index}`} className="owner-combo-equipmentRow">
            <div className="owner-combo-equipmentRow__media">
              {imageUrl ? <img src={imageUrl} alt={equipment.name || `Thiết bị ${index + 1}`} /> : <span>{comboPlaceholder(equipment.name)}</span>}
            </div>
            <div className="owner-combo-equipmentRow__body">
              ...
              <div className="owner-combo-equipmentRow__desc">{equipment.description || item.note || "Thiết bị được lấy từ catalog gốc của hệ thống."}</div>
            </div>
          </div>
        );
      })}
      {!items.length ? <div className="owner-combo-empty">Combo này chưa có thiết bị hiển thị.</div> : null}
    </div>
  );
    ...
    <div className="owner-combo-page">
      <section className="owner-combo-hero">
        <div>
          ...
          <p>
            Owner chỉ mua theo combo, nhưng vẫn xem rõ từng thiết bị có trong combo để gửi yêu cầu chính xác,
            sau đó thanh toán 2 giai đoạn qua PayOS.
          </p>
        </div>
        ...
        <section className="owner-combo-panel">
          <div className="owner-combo-panel__header">
            <div>
              ...
              <p>Nhấn vào một combo để xem đầy đủ thiết bị thành phần trước khi gửi yêu cầu.</p>
            </div>
          </div>
          ...
          <div className="owner-combo-grid">
            {combos.map((combo) => {
              ...
              return (
                <article key={combo.id} className={`owner-combo-card ${isSelected ? "is-selected" : ""}`}>
                  <div className="owner-combo-card__hero">
                    <div className="owner-combo-card__media">
                      {thumbnailUrl ? <img src={thumbnailUrl} alt={combo.name} /> : <span>{comboPlaceholder(combo.name)}</span>}
                    </div>
                    <div className="owner-combo-card__content">
                      <div className="owner-combo-card__head">
                        <div>
                          <div className="owner-combo-card__title">{combo.name}</div>
                          <div className="owner-combo-card__meta">
                            {combo.code || `COMBO-${combo.id}`} · Supplier: {combo.supplier?.name || "-"}
                          </div>
                        </div>
                        <div className="owner-combo-card__price">{money(combo.price)}đ</div>
                      </div>

                      <div className="owner-combo-card__desc">{combo.description || "Không có mô tả combo."}</div>
                  ...
                  <div className="owner-combo-card__actions">
                    <button
                      ...
                      onClick={() => {
                        setForm((prev) => ({ ...prev, comboId: String(combo.id) }));
                    ...
                    >
                      {isExpanded ? "Ẩn danh sách thiết bị" : "Xem thiết bị trong combo"}
                    </button>
                  </div>
            ...
            {!combos.length ? <div className="owner-combo-empty">Chưa có combo active.</div> : null}
          </div>
        </section>

        <form onSubmit={submit} className="owner-combo-panel owner-combo-formPanel">
          <div className="owner-combo-panel__header">
            <div>
          ...
          <div className="owner-combo-formGrid">
            <select className="owner-combo-input" value={form.comboId} onChange={(e) => setForm({ ...form, comboId: e.target.value })}>
              <option value="">Chọn combo</option>
              {combos.map((combo) => <option key={combo.id} value={combo.id}>{combo.name}</option>)}
            </select>
            <select className="owner-combo-input" value={form.gymId} onChange={(e) => setForm({ ...form, gymId: e.target.value })}>
              <option value="">Chọn gym / branch</option>
              {gyms.map((gym) => <option key={gym.id} value={gym.id}>{gym.name}</option>)}
            </select>
            <input className="owner-combo-input" placeholder="Tên liên hệ" value={form.contactName} onChange={(e) => setForm({ ...form, contactName: e.target.value })} />
            <input className="owner-combo-input" placeholder="Số điện thoại" value={form.contactPhone} onChange={(e) => setForm({ ...form, contactPhone: e.target.value })} />
            <input className="owner-combo-input owner-combo-formGrid__full" placeholder="Email liên hệ" value={form.contactEmail} onChange={(e) => setForm({ ...form, contactEmail: e.target.value })} />
            <textarea className="owner-combo-input owner-combo-input--textarea owner-combo-formGrid__full" placeholder="Ghi chú / lý do mua" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} />
          </div>

          <div className="owner-combo-pricing">
            <div className="owner-combo-pricing__row"><span>Tổng giá combo</span><strong>{money(comboTotals(selectedCombo).total)}đ</strong></div>
            <div className="owner-combo-pricing__row"><span>Tiền cọc 30%</span><strong>{money(comboTotals(selectedCombo).deposit)}đ</strong></div>
            <div className="owner-combo-pricing__row"><span>Tiền còn lại 70%</span><strong>{money(comboTotals(selectedCombo).final)}đ</strong></div>
          </div>

          {selectedCombo ? (
            <div className="owner-combo-selectedPreview">
              <div className="owner-combo-selectedPreview__title">Preview dữ liệu owner sẽ gửi sang admin</div>
              <div className="owner-combo-selectedPreview__hero">
                <div className="owner-combo-selectedPreview__media">
                  {absUrl(selectedCombo.thumbnail) ? <img src={absUrl(selectedCombo.thumbnail)} alt={selectedCombo.name} /> : <span>{comboPlaceholder(selectedCombo.name)}</span>}
                </div>
                <div>
                  <strong>{selectedCombo.name}</strong>
                  <div className="owner-combo-card__meta">{selectedCombo.code || `COMBO-${selectedCombo.id}`} · {selectedCombo.supplier?.name || "Chưa có supplier"}</div>
                </div>
              </div>
          ...
          <button className="owner-combo-btn owner-combo-btn--accent owner-combo-btn--full" type="submit">
      ...
      <section className="owner-combo-panel owner-combo-history">
        <div className="owner-combo-panel__header">
          <div>
            ...
            <p>Mỗi request đều hiện trạng thái, số tiền cọc, số tiền còn lại và snapshot thiết bị đúng tại thời điểm mua.</p>
          </div>
        </div>

        {loading ? <div className="owner-combo-empty">Đang tải dữ liệu...</div> : null}
        <div className="owner-combo-historyList">
          {requests.map((request) => {
            const combo = request.combo || {};
            const thumbnailUrl = absUrl(combo.thumbnail);
            return (
              <article id={`owner-purchase-request-${request.id}`} key={request.id} className="owner-combo-historyCard">
                <div className="owner-combo-historyCard__hero">
                  <div className="owner-combo-historyCard__media">
                    {thumbnailUrl ? <img src={thumbnailUrl} alt={combo.name || request.code} /> : <span>{comboPlaceholder(combo.name || request.code)}</span>}
                  </div>
                  <div className="owner-combo-historyCard__main">
                    <div className="owner-combo-historyCard__top">
                      <div>
                        <div className="owner-combo-historyCard__code">{request.code}</div>
                        <div className="owner-combo-historyCard__meta">{combo.name || "-"} · {request.gym?.name || "-"}</div>
                      </div>
                      <div className="owner-combo-historyCard__topRight">
                        <span className={statusClass(request.status)}>{statusLabel(request.status)}</span>
                        <strong>{money(request.totalAmount || request.payableAmount)}đ</strong>
                      </div>
                    </div>
                    ...
                    <div className="owner-combo-historyGrid">
                      <div className="owner-combo-infoBlock">
                        <h4>Thanh toán</h4>
                        <div className="owner-combo-kv"><span>Cọc 30%</span><b>{money(request.depositAmount)}đ</b></div>
                        <div className="owner-combo-kv"><span>Còn lại 70%</span><b>{money(request.finalAmount || request.remainingAmount)}đ</b></div>
                      </div>
                      <div className="owner-combo-infoBlock">
                        <h4>Liên hệ / ghi chú</h4>
                        <div className="owner-combo-kv"><span>Người liên hệ</span><b>{request.contactName || "-"}</b></div>
                        <div className="owner-combo-kv"><span>SĐT / Email</span><b>{request.contactPhone || request.contactEmail || "-"}</b></div>
                        <div className="owner-combo-kv"><span>Ghi chú</span><b>{request.note || "-"}</b></div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="owner-combo-historyCard__actions">
                  {request.status === "approved_waiting_deposit" ? (
                    <button className="owner-combo-btn owner-combo-btn--accent" onClick={() => payPhase(request.id, "deposit")} disabled={payingId === request.id}>
                      Thanh toán cọc 30%
                    </button>
                  ) : null}
                  {request.status === "shipping" ? (
                    <button className="owner-combo-btn owner-combo-btn--accent" onClick={() => confirmReceived(request.id)}>
                      Xác nhận đã nhận combo
                    </button>
                  ) : null}
                  {request.status === "delivered_waiting_final_payment" ? (
                    <button className="owner-combo-btn owner-combo-btn--accent" onClick={() => payPhase(request.id, "final")} disabled={payingId === request.id}>
                      Thanh toán 70% còn lại
                    </button>
                  ) : null}
                  {request.status === "rejected" ? (
                    <div className="owner-combo-rejectReason">Lý do từ chối: {request.rejectReason || request.adminRejectionNote || "-"}</div>
                  ) : null}
                </div>
              </article>
            );
          })}
          {!requests.length && !loading ? <div className="owner-combo-empty">Bạn chưa có yêu cầu mua combo nào.</div> : null}
        </div>
      </section>
    </div>
  );
