...
const getRescheduleMeta = (booking) => {
  ...
  if (booking?.pendingRescheduleRequest) {
    return {
      ...
      detail: `Đề xuất ${fmtDate(req.requestedDate)} · ${fmtTime(req.requestedStartTime)} - ${fmtTime(req.requestedEndTime)}`,
    };
  }
...
export default function BookingDetailModal({ booking, onClose, onUpdated }) {
  ...
  useEffect(() => {
    if (!showRequestForm || !booking?.id) return;
    let mounted = true;
    const run = async () => {
      ...
      try {
        ...
        const filteredDates = (data?.availableDates || []).filter((it) => getTrainerWorkingRanges(booking, it.date).length);
        const nextDate =
  ...
  }, [showRequestForm, booking, booking?.id, weekday, selectedDate, selectedStartTime]);
  ...
  const filteredAvailableDates = useMemo(() => {
    return (options?.availableDates || []).filter((it) => getTrainerWorkingRanges(booking, it.date).length);
  }, [booking, options]);
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      ...
      setSuccessMessage("Đã gửi yêu cầu đổi lịch đến PT. Bạn sẽ nhận được thông báo khi PT phản hồi.");
      ...
      <div className="bd-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <div className="bd-head">
          <div>
            ...
            <p className="bd-sub">Xem nhanh thông tin lịch tập và gửi yêu cầu đổi lịch ngay trong ứng dụng.</p>
          </div>
        ...
        {showRequestForm ? (
          <div className="bd-requestPanel">
            <div className="bd-requestHead">
              <RefreshCcw size={16} />
              <span>Yêu cầu đổi lịch</span>
            </div>
            <p className="bd-requestHint">
              Chỉ hiển thị các ngày và khung giờ nằm trong giờ làm việc của PT. Những khung giờ bận hoặc không hợp lệ sẽ không xuất hiện.
            </p>
            ...
            <div className="bd-pickerSection">
              ...
              <div className="bd-optionGrid">
                ...
                {!loadingOptions && !filteredAvailableDates.length ? (
                  <div className="bd-emptyState">PT hiện chưa có ngày phù hợp trong thời gian tới.</div>
                ) : null}
              </div>
            </div>
            ...
            <div className="bd-pickerSection">
              ...
              <div className="bd-slotGrid">
                ...
                {!loadingOptions && selectedDate && !filteredAvailableSlots.length ? (
                  <div className="bd-emptyState">Ngày này không còn khung giờ trống trong giờ làm việc của PT.</div>
                ) : null}
              </div>
            </div>
            ...
            {loadingOptions ? <div className="bd-inlineNotice">Đang cập nhật danh sách ngày và khung giờ khả dụng…</div> : null}
        ...
        <div className="bd-actions bd-actions--spread">
          ...
          {!showRequestForm && canRequestReschedule ? (
            <button
          ...
          {booking?.pendingRescheduleRequest && !showRequestForm ? (
            <button className="bd-btn bd-btn--disabled" type="button" disabled>
              Đang chờ phản hồi
            </button>
          ) : null}
          {!showRequestForm && !booking?.pendingRescheduleRequest && !canRequestReschedule ? (
            <button
              className="bd-btn bd-btn--disabled"
              type="button"
              disabled
              title={isPastBooking ? "Buổi tập đã qua nên không thể yêu cầu đổi lịch" : "Buổi tập này hiện không thể đổi lịch"}
            >
          ...
          {showRequestForm ? (
            <>
              ...
              <button className="bd-btn" onClick={handleSubmitRequest} type="button" disabled={submitting || loadingOptions}>
